...
/**
 * Form controller for Jalur prestasi edit forms.
 *
 * @ingroup jalur_prestasi
 */
class JalurPrestasiForm extends ContentEntityForm {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
	...
	if(is_null($entity->id())){
	  ...
	  $id = \Drupal::entityQuery('jalur_prestasi')
	        ->condition('name', $form_state->getValue('name')[0]['value'])
			->condition('id', $entity->id(), '!=')
			->range('0', '1')
			->execute();
	  if(!empty($id)){
	    $form_state->setErrorByName('name',t("The jalur prestasi with name @name already exist", array('@name' => $form_state->getValue('name')[0]['value'])));
	  }
	}
  }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        drupal_set_message($this->t('Created the %label Jalur prestasi.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        drupal_set_message($this->t('Saved the %label Jalur prestasi.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.jalur_prestasi.canonical', ['jalur_prestasi' => $entity->id()]);
  }
